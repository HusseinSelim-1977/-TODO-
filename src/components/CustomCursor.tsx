...
export function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const handleMouseOver = (e: MouseEvent) => {
      ...
      if (isInteractive) {
        setCursorState({ scale: 1.5, opacity: 0.6 })
      } else {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[9999] hidden md:block">
      ...
      <motion.div
        ...
        transition={{
          scale: { type: "spring", damping: 25, stiffness: 400, mass: 0.3 },
          opacity: { duration: 0.2, ease: [0.2, 0.8, 0.3, 1.0] as [number, number, number, number] },
        }}
